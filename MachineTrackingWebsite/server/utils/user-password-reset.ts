...
export function createUserResetToken(user: { id: string; email: string; password_hash: string }): string {
...
export async function verifyUserResetToken(token: string) {
  ...
  if (user.password_hash.slice(0, 20) !== payload.pwh) return null

  return user
}

export function getUserResetTtlMinutes(): number {
