...
<template>
  <div class="admin-reset">
    <form class="admin-reset__card" @submit.prevent="resetPassword">
      <h1>Reset Admin Password</h1>
      <p>Enter your reset token and set a new password.</p>

      <label class="admin-reset__field">
      ...
      <p v-if="errorMessage" class="admin-reset__error">{{ errorMessage }}</p>
      <p v-if="successMessage" class="admin-reset__success">{{ successMessage }}</p>

      <button type="submit" :disabled="loading">
        {{ loading ? 'Resetting...' : 'Reset Password' }}
      </button>

      <NuxtLink to="/admin/login" class="admin-reset__link">Back to Admin Login</NuxtLink>
    </form>
  </div>
</template>
...
.admin-reset__field input:focus {
  outline: none;
...
.admin-reset__success {
  color: var(--primary-hover);
}

.admin-reset__card button {
...
.admin-reset__card button:hover:not(:disabled) {
  filter: brightness(1.06);
  box-shadow: 0 8px 20px rgba(128, 176, 224, 0.25);
}

.admin-reset__card button:disabled {
  opacity: 0.7;
  cursor: not-allowed;
}
